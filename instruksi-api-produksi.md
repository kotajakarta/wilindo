# Instruksi Penggunaan API Wilayah — Production

API wilayah administratif Indonesia (provinsi, kabupaten/kota, kecamatan,
desa/kelurahan) berjenjang ini **sudah live di production**. Dokumen ini
untuk developer **atau AI coding assistant** yang mengintegrasikan
aplikasi lain (Node.js, PHP, bahasa apa pun yang bisa HTTP request) dengan
API ini — bisa dibaca dan langsung dieksekusi tanpa konteks tambahan,
tidak perlu install/jalankan apa pun sendiri.
...
Lihat **"Pola Integrasi Dropdown Bertingkat"** di bawah untuk alur
...
5. Uji alur lengkap dengan data asli sebelum menganggap selesai (mis. pilih provinsi nyata → pastikan kabupaten/kecamatan/desa ter-load benar) — API ini sudah live, tidak perlu server lokal untuk mencobanya.
